...
-- 883
day5aAnswer :: Integer
day5aAnswer = maximum $ fmap day5a Inputs.day5
...
sortedSeatIDs :: [Integer]
sortedSeatIDs = sort $ fmap day5a Inputs.day5

-- |
-- >>> day5bAnswer (sort sortedSeatIDs)
-- Just 532
day5bAnswer :: [Integer] -> Maybe Integer
...
-- >>> decode "BBFFBBFRLL"
-- (102,4)
decode :: [Char] -> (Integer, Integer)
decode a = (fst (decodeRows rows), fst (decodeCols columns))
    where
        rows = take 7 a
        columns = drop 7 a

-- decodeCols "RLR" column 5
-- decodeCols "RRR" column 7
-- decodeCols "RLL" column 4
decodeCols :: Foldable t => t Char -> (Integer, Integer)
decodeCols  = foldl decodeColsWithHalves (0 :: Integer, 7 :: Integer)

decodeColsWithHalves :: (Integer, Integer) -> Char ->  (Integer, Integer)
decodeColsWithHalves (mini, maxi) r 
...
-- decodeRows "BBFFBBF"
-- (102,102)
decodeRows :: Foldable t => t Char -> (Integer, Integer)
decodeRows = foldl decodeRowsWithHalves (0 :: Integer, 127 :: Integer)

decodeRowsWithHalves :: (Integer, Integer) -> Char ->  (Integer, Integer)
decodeRowsWithHalves (mini, maxi) r 
      | r == 'F' = (mini, (mini + maxi) `div` 2)
      | r == 'B' = ((mini + maxi) `div` 2 + 1, maxi)
      | otherwise = error "the impossible happened"
